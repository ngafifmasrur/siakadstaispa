...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $majors = AdmissionRegistrant::groupBy('major1')
            ->whereHas('admission', function ($admissin) {
                return $admissin->opened();
            })->select('major1', DB::raw('count(*) AS total'))
            ->get();

        $second_majors = AdmissionRegistrant::groupBy('major2')
            ->whereHas('admission', function ($admissin) {
                return $admissin->opened();
            })->select('major2', DB::raw('count(*) AS total'))
            ->get();
    ...
    public function getStatistics()
    {
        return [
            'customers.index' => [
                ...
                'color' => 'primary',
                'data' => AdmissionRegistrant::whereHas('admission', function ($admissin) {
                    return $admissin->opened();
                })->count(),
            ],
            'consumers.index' => [
    ...
    public function getDetailQuota()
    {
        ...
        foreach (array_keys(UserProfile::$sex) as $sex => $sexName) {
            foreach ($admissions as $a) {
                $data['Total pendaftar'][$sex] = $a->registrants()
                    ->whereHas('user.profile', function ($profile) use ($sex) {
                        return $profile->where('sex', $sex);
                    })->count();

                $data['Terverifikasi'][$sex] = $a->registrants()
                    ->whereNotNull('verified_at')->whereHas('user.profile', function ($profile) use ($sex) {
                        return $profile->where('sex', $sex);
                    })->count();

                $data['Sudah bayar'][$sex] = $a->registrants()
                    ->whereNotNull('verified_at')->whereNotNull('tested_at')
                    ->has('transactions')
                    ->whereHas('user.profile', function ($profile) use ($sex) {
                        return $profile->where('sex', $sex);
                    })->count();

                $data['Belum lunas'][$sex] = $a->registrants()
                    ->whereNotNull('verified_at')->whereNotNull('tested_at')
                    ->has('transactions')->whereNull('paid_off_at')
                    ->whereHas('user.profile', function ($profile) use ($sex) {
                        return $profile->where('sex', $sex);
                    })->count();

                $data['Lunas'][$sex] = $a->registrants()
                    ->whereNotNull('verified_at')->whereNotNull('tested_at')
                    ->has('transactions')->whereNotNull('paid_off_at')
                    ->whereHas('user.profile', function ($profile) use ($sex) {
                        return $profile->where('sex', $sex);
                    })->count();

                $data['Santri Mandiri'][$sex] = $a->registrants()
                    ->where('is_saman', 1)->whereHas('user.profile', function ($profile) use ($sex) {
                        return $profile->where('sex', $sex);
                    })->count();
                $data['Reguler'][$sex] = $a->registrants()
                    ->where('is_saman', 0)->whereHas('user.profile', function ($profile) use ($sex) {
                        return $profile->where('sex', $sex);
                    })->count();
            }
        }
