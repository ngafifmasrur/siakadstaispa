...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-8">
            <h2 class="mb-0"><a class="text-decoration-none small" href="{{ request('next', url()->previous()) }}"><i class="mdi mdi-arrow-left-circle"></i></a> Tambah data kamar</h2>
            <hr>
            <div class="card">
                <div class="card-body">
                    <form class="form-block" action="{{ route('admission.admin.database.manage.rooms.store', ['next' => request('next', url()->previous())]) }}" method="POST"> @csrf
                        <fieldset>
                                ...
                                <div class="col-md-7">
                                    <select class="form-control" name="area">
                                        @foreach($areas as $i => $area)
                                            <option value="{{ $i }}" @if(request('area') == $i) selected @else disabled @endif> {{ $area }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group required row">
                                ...
                                <div class="col-md-3">
                                    <input type="number" class="form-control @error('quota') is-invalid @enderror " name="quota" value="{{ old('quota') }}" min="0" max="100" required>
                                    @error('quota')
                                        <span class="invalid-feedback"> {{ $message }} </span>
                                    @enderror
                                </div>
