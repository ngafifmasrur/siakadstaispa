...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Alamat asal</li>
@endsection

@section('content')
    <h2 class="mb-0"><a class="text-decoration-none small" href="{{ request('next', url()->previous()) }}"><i class="mdi mdi-arrow-left-circle"></i></a> Ubah data {{ $trans }}</h2>
    <hr>
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <form class="form-block" action="{{ route('admission.admin.database.manage.registrants.update', ['type' => $type , 'registrant' => $registrant->id, 'uid' => $registrant->user_id, 'key' => request('key'), 'next' => request('next')]) }}" method="POST" enctype="multipart/form-data"> @csrf @method('PUT')
