...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Tahap tes</li>
@endsection
...
@section('content')
    <div class="card">
        ...
        <div class="card-body bg-light border-top">
            <form class="form-block" id="search-form" action="{{ route('admission.admin.registration.test.index') }}" method="GET">
                ...
                <div class="form-inline">
                    ...
                    <div class="my-1 mr-sm-2">
                        ...
                        <a class="btn btn-secondary" href="{{ route('admission.admin.registration.test.index') }}"><i class="mdi mdi-refresh"></i> Refresh</a>
                    </div>
                </div>
            </form>
        </div>
        @if(request('search'))
            <div class="card-body border-top text-muted py-2 alert-info">
                Jika calon mahasiswa yang Anda cari tidak ada, kemungkinan calon mahasiswa tersebut belum terverifikasi.
            </div>
                    ...
                    <tbody>
                        @forelse($registrants as $registrant)
                            <tr class="{{ empty($registrant->special) ?: 'table-warning' }}">
                                ...
                                @can('testRegistrant', AdmissionRegistrant::class)
                                    <td class="py-2 align-middle border-left text-center" nowrap>
                                        <a class="btn btn-success btn-sm" href="{{ route('admission.admin.registration.test.show', ['registrant' => $registrant->id]) }}"><i class="mdi mdi-format-list-bulleted"></i> Lihat detail </a>
                                        @if($registrant->tested_at)
                                            <a class="btn btn-success btn-sm" href="{{ route('admission.admin.registration.test.print', ['registrant' => $registrant->id]) }}" target="_blank"><i class="mdi mdi-printer"></i> Cetak</a>
                                        @endif
                                    </td>
                                @endcan
                            </tr>
