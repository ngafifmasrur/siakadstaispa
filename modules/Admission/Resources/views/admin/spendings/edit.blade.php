...
<div class="row justify-content-center">
    <div class="col-8">
        <h2 class="mb-0"><a class="text-decoration-none small" href="{{ request('next', url()->previous()) }}"><i class="mdi mdi-arrow-left-circle"></i></a> Ubah data</h2>
        <hr>
        <div class="card">
            <div class="card-body">
                <form class="form-block" action="{{ route('admission.admin.spendings.update', ['spending' => $spending->id]) }}" method="POST" enctype="multipart/form-data"> @csrf @method('PUT')
                    <fieldset>
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group required">
                                    <label>Nama item</label>
                                    <input type="text" class="form-control @error('item') is-invalid @enderror" name="item" value="{{ old('item', $spending->item) }}" required>
                                    @error('item')
                                ...
                                <div class="form-group required">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('amount') is-invalid @enderror" name="amount" value="{{ old('amount', $spending->amount) }}" required>
                                        ...
                                        <select class="custom-select" name="unit" @error('unit') is-invalid @enderror>
                                            @foreach(\Modules\Admission\Models\AdmissionSpending::$unit as $id => $unit)
                                                <option value="{{ $id }}" @if($spending->unit == $id) selected @endif>{{ $unit }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group required">
                                    <label>Qty</label>
                                    <input type="number" class="form-control @error('qty') is-invalid @enderror" name="qty" value="{{ old('qty', $spending->qty) }}" required>
                                    @error('qty')
                                        <span class="invalid-feedback"> {{ $message }} </span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-sm-6">
                                        ...
                                        @foreach($admissions as $admission)
                                            <optgroup label="{{ $admission->full_name }}">
                                                @foreach($admission->committees as $committee)
                                                    <option value="{{ $committee->id }}" @if($spending->committee_id == $committee->id) selected @endif>{{ $committee->name }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group required">
                                    ...
                                    <input type="text" class="form-control @error('shop') is-invalid @enderror" name="shop" value="{{ old('shop', $spending->shop) }}" required>
                                    @error('shop')
                                        <span class="invalid-feedback"> {{ $message }} </span>
                                    @enderror
                                </div>
                                <div class="form-group required">
                                    ...
                                    <input type="text" class="form-control @error('buyer') is-invalid @enderror" name="buyer" value="{{ old('buyer', $spending->buyer) }}" required>
                                    @error('buyer')
                                        <span class="invalid-feedback"> {{ $message }} </span>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group required">
                            ...
                            <div class="input-group date" id="payed_at" data-target-input="nearest">
                                <input type="text" class="form-control datetimepicker-input @error('payed_at') is-invalid @enderror" name="payed_at" value="{{ old('payed_at', $spending->payed_at) }}" data-toggle="datetimepicker" data-target="#payed_at">
