...
<x-card>
    <form action="{{ route('admin_prodi.kurikulum_prodi.store') }}" method="POST">
        @csrf
        <div class="row">
            <div class="form-group col-lg-3">
    ...
    @foreach ($table_semester as $item)
            <div class="col-lg-6">
                ...
                <x-semester-table
                    :title="'Semester '.$item"
                    :id="'semester'.$item"
                    :route="route('admin_prodi.kurikulum_prodi.data_index', [ 'tahun_ajaran' => $tahun_ajaran->id_tahun_ajaran])" 
                    :table="[
