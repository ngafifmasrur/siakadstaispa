...
<head>
    ...
    <title>REKAP PRESENSI DOSEN {{ $dosen->nama_dosen }}</title>
    <style>
        html {
            border: 0px;
            margin: 20px;
...
<body>
    ...
    <h4 align="center" style="margin-bottom: 10px!important;margin-top: 0px!important;text-decoration: underline;">PRESENSI: {{ $dosen->nama_dosen }}</h4>
    @php
        ...
        @foreach ($list_kelas as $kelas)
            <tr>
                ...
                <td align="center" class="border border-gray-400">
                    ...
                    @switch($jurnal->where('id_kelas_kuliah', $kelas->id_kelas_kuliah)->where('pertemuan_ke', $i)->first()->status ?? '-')
                        @case("Hadir")
                            H
                            @break
                        @case('Sakit')
